...
//				x.sendMsgToPlayer(WorldPb.SynEntityRq.ext, old.build(), WorldPb.SynEntityRq.EXT_FIELD_NUMBER);
			});
		...
		synCountryInfo();
		heroManager.synBattleScoreAndHeroList(player, player.getAllHeroList());
		seasonService.addTreasuryScore(player, GrandType.TYPE_3, 2, 1);
	}
...
//			player.sendMsgToPlayer(FlameWarPb.SynFlameCountryInfo.ext, builder.build(), FlameWarPb.SynFlameCountryInfo.EXT_FIELD_NUMBER);
	...
	public void playerFly(Player player) {
		...
		PlayerCity playerCity = worldManager.removePlayerCity(oldPos, currentMapInfo);
		flameMap.removeNode(playerCity);

		playerManager.changePlayerPos(player, randPos);
		PlayerCity playerCity1 = worldManager.addPlayerCity(randPos, currentMapInfo, player);
		flameMap.addNode(playerCity1);
		synFlameEntity(playerCity1, oldPos);
...
//			player1.sendMsgToPlayer(FlameWarPb.SynFlameEntityRq.ext, builder.build(), FlameWarPb.SynFlameEntityRq.EXT_FIELD_NUMBER);
	...
	public void logOutMap(ClientHandler handler) {
		...
		if (playerCity1 != null) {
			...
			flameMap.removeNode(playerCity1);
