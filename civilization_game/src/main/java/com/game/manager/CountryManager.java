...
@Component
@LoadData(name = "国家信息", type = Loading.LOAD_USER_DB, initSeq = 1800)
...
//        chatManager.sendWorldChat(ChatId.PRO_MILI, params);
	 ...
	 * @return
	 */
	public CommonPb.Resource.Builder getResourceAdd(CommonPb.Resource.Builder base, int country) {
		StaticCountry staticCountry = staticCountryMgr.getCountryMap().get(country);
		if (staticCountry == null) {
			return null;
		}

		CommonPb.Resource.Builder additon = CommonPb.Resource.newBuilder();

		for (List<Integer> list : staticCountry.getResouceAddtion()) {
			if (list.isEmpty() || list.size() != 2) {
				continue;
			}
			int resType = list.get(0);
			int effectValue = list.get(1);
	 ...
	 * @return
	 */
	public double getSoldierSpeed(int country, int soilderType) {
		StaticCountry staticCountry = staticCountryMgr.getCountryMap().get(country);
		if (staticCountry == null) {
			return 0.0d;
		}

		for (List<Integer> list : staticCountry.getSodierAddtion()) {
			if (list.isEmpty() || list.size() != 2) {
				continue;
			}
			if (soilderType == list.get(0)) {
				int effectValue = list.get(1);
				return effectValue / 100.00d;
			}
