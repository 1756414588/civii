...
import com.game.season.seasongift.entity.SeasonCardInfo;
...
public class ActivityManager {
	...
	public boolean updActPerson(Player player, ActivityBase activityBase, long schedule, int sortId) {
		try {
			...
			if (activityBase.getActivityId() == ActivityConst.ACT_RAIDERS || activityBase.getActivityId() == ActivityConst.ACT_LUCKLY_EGG || activityBase.getActivityId() == ActivityConst.RE_DIAL) {
				ActivityEventManager.getInst().activityTip(player, activity, activityBase);
	...
	public void updActServer(int activityId, long schedule, int sortId) {
		try {
			...
			if (newOpen) {
				ActivityEventManager.getInst().activityTip(EventEnum.ACT_SERVER, new ActServerCensusActor(activityBase, actData));
			}
	...
	public void actHighVipPush() {
		...
		if (newOpen) {
			...
			ActivityEventManager.getInst().activityTip(EventEnum.ACT_SERVER, new ActServerCensusActor(activityBase, activityData));
		}
	}
...
//		LogHelper.MESSAGE_LOGGER.info("actPayGift");
		ActivityEventManager.getInst().activityTip(EventEnum.ACT_BUY_GIFT, new CommonTipActor(player, actRecord, activityBase));
	...
	public boolean actPayCard(Player player, int cardId) {
		...
		if (payCard.getCardType() == 5) {
			SeasonCardInfo module = player.getModule(SeasonCardInfo.class);
			module.setEndTime(System.currentTimeMillis() + TimeHelper.MONTH_MS);
			List<Award> awards = new ArrayList<>();
			for (List<Integer> list : payCard.getSellList()) {
				awards.add(new Award(list.get(0), list.get(1), list.get(2)));
			}
			module.setDrawTime(TimeHelper.curentTime());
			playerManager.sendAttachMail(player, Lists.newArrayList(new Award(AwardType.GOLD, 0, payCard.getDiamond())), MailId.SEASON_MAIL_167, String.valueOf(payCard.getPayCardId()), String.valueOf(payCard.getPayCardId()));
			playerManager.sendAttachMail(player, awards, MailId.SEASON_MAIL_168, String.valueOf(payCard.getPayCardId()), String.valueOf(payCard.getPayCardId()), String.valueOf(payCard.getPeriod()));
		}
...
//		LogHelper.MESSAGE_LOGGER.info("购买通行证 通知客户端:{}", player.getRoleId());
	...
	public void calcuKillAll(Team attack, Team defence) {
		...
		playerKill.forEach((lordId, killNum) -> {
			...
			if (player != null) {
				...
				ActivityEventManager.getInst().updateActivityHandler(EventEnum.KILL_PLAYER_SOIDLER, new CommonTipActor(player, actRecord, activityBase));
			}
		});
	}
	...
	public void updateActDailyTrain(Player player, int soidierCount) {
		...
		ActivityEventManager.getInst().updateActivityHandler(EventEnum.TRAINR, new CommonTipActor(player, actRecord, activityBase));
...
//					LogHelper.MESSAGE_LOGGER.info("updActWellCrownThreeArmy");
	...
	public void updateActMonster(Player player) {
		...
		actRecord.addCount();
		ActivityEventManager.getInst().updateActivityHandler(EventEnum.KILL_MONSTER, new CommonTipActor(player, actRecord, activityBase));
	}
	...
	public void updateActInvestRq(Player player) {
		...
		// 已参与则领奖
		if (state != 0) {
			ActivityEventManager.getInst().activityTip(player, actRecord, getActivityBase(ActivityConst.ACT_MONSTER));
		}
	}
...
//		LogHelper.MESSAGE_LOGGER.info("flushShowBeauty");
	...
	public boolean refreshTDTaskTips(Player player, ActRecord actRecord) {
		...
		Map<Integer, Integer> received = actRecord.getReceived();
		for (StaticTDSevenBoxAward value : tdTaskManager.getStaticTDSevenBoxAwardMap().values()) {
