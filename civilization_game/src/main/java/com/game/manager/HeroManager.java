...
//    }
    ...
    public void caculateProp(Hero hero, Player player) {
        ...
        // 美女加成
        int beautyAddition = getBeautyAddition(hero, player, qulifyAdd, ctProperty);
        // 科技加成（包括兵种进阶）
        techManager.getHeroProperty(player, getSoldierType(hero.getHeroId()), ctProperty);

        //赛季天赋
        seasonManager.addProperty(player, ctProperty);

        int soldierByPercentage = (int) Math.floor((baseAdd.getSoldierNum() / 4.0 * soldierLines + qulifyAdd.getSoldierNum()) * (1 + beautyAddition / DevideFactor.PERCENT_NUM));
        ...
        double seasonBuf = seasonManager.getSeasonBuf(player, EffectType.EFFECT_TYPE2);
        int v2 = (int) Math.ceil(total.getAttack() * seasonBuf);
        total.addAttackCity(v2);
    ...
    public Hero addHero(Player player, int heroId, int reason) {
        ...
        if (hero.getType() > 0) {
            seasonService.addSevenScore(SevenType.HERO, 1, player, 1);
            seasonService.addJourney(player, JourneyType.HERO, 1, heroId);
        }
        return hero;
    }
    ...
    public boolean addExp(Hero hero, Player player, long exp, int reason) {
		...
		if (hero.getType() > 0) {
			seasonService.addSevenScore(SevenType.HERO_LEVEL, hero.getHeroLv(), player, 1);
			seasonService.addJourney(player, JourneyType.UP_LEVEL, hero.getHeroLv(), hero.getHeroId());
		}
    ...
    ////美女英雄加成
    //public int getBeautyAddition(Hero hero, Player player, Property qulifyAdd) {
    //    Property property = new Property();
    //    StaticHero staticHero = staticHeroDataMgr.getStaticHero(hero.getHeroId());
    //    if (staticHero == null) {
    //        return 0;
    ...
    //    return percentageOfForceAdition;
    //}

    //美女英雄加成
    public int getBeautyAddition(Hero hero, Player player, Property qulifyAdd, Property property) {
    ...
    public void caculateAllProperty(HashMap<Integer, Hero> heroes, Player player) {
        if (heroes == null) {
            return;
        }
...
//            double missFactor = (double) staticLimitMgr.getNum(273) / DevideFactor.PERCENT_NUM;
    ...
    public List<Integer> checkHeroChange(Player player, List<Integer> heroList) {
        ...
        HashMap<Integer, Hero> heroMap = player.getHeros();
    ...
    public Integer heroChanageId(HashMap<Integer, Hero> heroMap, List<StaticHero> typeList, int heroId) {
