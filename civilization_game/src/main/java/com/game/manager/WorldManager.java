...
import com.game.worldmap.CityInfo;
...
//    private Map<Integer, Map<Integer, WarAssemble>> war = new HashMap<>();//key1 mapId  key2 阵营id
    ...
    public void loadWorldPos() {
        ...
        for (Map.Entry<Integer, StaticWorldMap> entry : staticWorldMapMap.entrySet()) {
            ...
            if (mapId != MapId.FIRE_MAP) {
                ...
                // 初始化坐标
                mapInfo.initPos(staticWorldMap.getX1(), staticWorldMap.getX2(), staticWorldMap.getY1(), staticWorldMap.getY2());
            } else {
    ...
    public void addPlayerInfo(Player player) {
        ...
        if (mapInfo.getCityInfos().get(pos) != null) {
            randomMoveCity(player);
            mapInfo.updatePlayerNum(player.getCountry());
            return;
        }
    ...
    // 删除玩家城池
    public PlayerCity removePlayerCity(Pos pos, MapInfo mapInfo) {
        Map<Pos, PlayerCity> playerCityMap = mapInfo.getPlayerCityMap();
        if (!playerCityMap.containsKey(pos)) {
            return null;
        }
        ...
        return playerCityMap.remove(pos);
    ...
    public Pos randPos(Player player, int configNum) {
        ...
        MapInfo mapInfo = getWorldMapInfo().get(mapId);
        if (mapInfo == null) {
        ...
        if (posSize <= 0) {
//            LogHelper.ERROR_LOGGER.error(" 0.");
            return pos;
        }

        int randPos = RandomHelper.threadSafeRand(1, posSize);
        int index = randPos - 1;
        Pos pickPos = aroundPos.get(index);
        if (!pickPos.isError()) {
            return pickPos;
        }
     ...
     * @return
     */
    public March createRebelAndAssistMarch(Player player, Player target, List<Integer> heroIds, Pos targetPos, int marchType) {
        March march = new March();
        march.setKeyId(marchManager.getMarchKey());
        march.setLordId(player.roleId);
        List<Integer> marchHero = march.getHeroIds();
        ...
        // 兵书对行军的影响
        float bookEffectMarch = warBookManager.getBookEffectMarch(player, heroIds);
        long period = getPeriod(player, target, playerPos, targetPos, bookEffectMarch, marchType);
        march.setPeriod(period);
        march.setEndTime(System.currentTimeMillis() + period);
        march.setCountry(player.getCountry());
    ...
    public long getPeriod(Player player, Pos pos1, Pos pos2, float bookEffectMarch) {
        if (getMapId(pos2) == MapId.FIRE_MAP) {
            long period = flameWarManager.getPeriod(player, pos1, pos2, bookEffectMarch);
            return period;
        ...
        long period = (long) Math.ceil(distance / (configNum * 1.0f * (1 + spAdd)) * (1 - vipFactor) * (1 - propAdd) * (1 - bookEffectMarch) / (1 + bigMonsterSpeed) * (1 - broodSpeed) * (1 - commandBuff) * (1 - cityBuf) * (1 - seasonBuf));
        //long period = (long) Math.ceil(distance / (configNum * 1.0f * (1 + spAdd)) * (1 - vipFactor) * (1 - propAdd) * (1 - bookEffectMarch) / (1 + bigMonsterSpeed) * (1 - broodSpeed) * (1 - commandBuff) * (1 - cityBuf)); * (1 - buff)
    ...
    // * @return
    // */
    //public long getAssistPeriod(Player player, Player target, Pos pos1, Pos pos2, float bookEffectMarch) {
    //    int distance = distance(pos1, pos2);
    //    float configNum = staticLimitMgr.getNum(10) * 1.0f / 1000;
    //    double spAdd = techManager.getArmySp(player);
    //    float vipFactor = playerManager.getMarchBuff(player);
    ...
    //        buff = flameWarService.getBuff(BuffType.buff_5, player.getCountry(), player);
    //    }
    //    double seasonBuf = seasonManager.getSeasonBuf(player, EffectType.EFFECT_TYPE_1);// 自己有加速buf
    //    double seasonBuf1 = seasonManager.getSeasonBuf(target, EffectType.EFFECT_TYPE14);// 驻防的人有buf
    //    long period = (long) Math.ceil(distance / (configNum * 1.0f * (1 + spAdd)) * (1 - vipFactor) * (1 - propAdd) * (1 - bookEffectMarch) / (1 + bigMonsterSpeed) * (1 - broodSpeed) * (1 - commandBuff) * (1 - cityBuf) * (1 - buff) * (1 - seasonBuf) * (1 - seasonBuf1));
    //    return period * TimeHelper.SECOND_MS;
    //}
     ...
     * @return
     */
    public long getPeriod(Player player, Player target, Pos pos1, Pos pos2, float bookEffectMarch, int marchType) {
        int distance = distance(pos1, pos2);
        float configNum = staticLimitMgr.getNum(10) * 1.0f / 1000;
        double spAdd = techManager.getArmySp(player);
        float vipFactor = playerManager.getMarchBuff(player);
        ...
        double seasonBuf = 0d;
        double seasonBuf1 = 0d;
        seasonBuf = seasonManager.getSeasonBuf(player, EffectType.EFFECT_TYPE_1);// 自己有加速buf
        if (marchType == MarchType.CityFriendAssist && target != null) {
            seasonBuf1 = seasonManager.getSeasonBuf(target, EffectType.EFFECT_TYPE14);// 驻防的人有buf
        }
        long period = (long) Math.ceil(distance / (configNum * 1.0f * (1 + spAdd)) * (1 - vipFactor) * (1 - propAdd) * (1 - bookEffectMarch) / (1 + bigMonsterSpeed) * (1 - broodSpeed) * (1 - commandBuff) * (1 - cityBuf) * (1 - buff) * (1 - seasonBuf) * (1 - seasonBuf1));//* (1 - seasonBuf)
    ...
    public boolean addPrimaryPos(Player player, Pos pos, PrimaryCollect primaryCollect) {
        ...
        mapInfo.addPos(pos, primaryCollect);
        playerManager.clearPos(pos);
        return true;
    }
    ...
    public void playerFly(Player player) {
        ...
        changePlayerPos(player, mapInfo, randPos);
//		handleWallFriendReturn(player);
        ...
        if (playerCity != null) {
            synEntityRq(playerCity, newMapInfo.getMapId(), oldPos); // 同步城池
        }
        ...
        if (staticWorldMapNew != null && staticWorldMapOld != null && staticWorldMapNew.getAreaType() == WorldAreaType.LOW_AREA) {
            if (staticWorldMapOld.getAreaType() == WorldAreaType.MIDDLE_AREA || staticWorldMapOld.getAreaType() == WorldAreaType.HIGH_AREA) {
                ...
                if (worldActPlan != null) {
                    SpringUtil.getBean(WorldActPlanService.class).syncWorldActivityPlan();
                }
            }
        }
    }
    ...
    public List<Award> getLost(Player target) {
        ...
        if (staticWare != null) {
            ...
            double seasonBuf = seasonManager.getSeasonBuf(target, EffectType.EFFECT_TYPE29);

            ironLimit = (long) (ironLimit * (1 + seasonBuf));
            copperLimit = (long) (copperLimit * (1 + seasonBuf));
            oilLimit = (long) (oilLimit * (1 + seasonBuf));
        }
...
//        oilDiff = Math.max(0, oilDiff);
    ...
    public void randerPlayerPos(Player player, boolean push) {
        ...
        if (pos.isError() || !mapInfo.isFreePos(pos)) {
            LogHelper.CONFIG_LOGGER.error("randerPlayerPos pos.isError() || isPosTaken(mapId, pos)!");
            return;
        }
        Pos oldPos = player.getOldPos();

        playerManager.changePlayerPos(player, pos);
        ...
        for (StaticWorldMap staticWorldMap : worldMap.values()) {
            ...
            for (int monsterLv = monsterMinLv; monsterLv <= monsterMaxLv; monsterLv++) {
                ...
                for (int monsterNum = 1; monsterNum <= maxNum; monsterNum++) {
                    ...
                    addMonster(monsterPos, 1000 + monsterLv, monsterLv, mapInfo, AddMonsterReason.FLUSH_INIT_MONSTER);
                }
            }
    ...
    public void flushInitActMonster() {
        ...
        for (StaticWorldMap staticWorldMap : worldMap.values()) {
            ...
            for (int num = 1; num <= maxNum; num++) {
                ...
                // 创建一个野怪
                addMonster(monsterPos, 1000 + monsterLv, monsterLv, mapInfo, AddMonsterReason.FLUSH_ACT_MONSTER);
                totalNum++;
                if (totalNum >= maxNum) {
                    break;
                }
    ...
    public boolean hasWorld(int worldId) {
        World world = worldDao.selectWorld(worldId);
        return world != null;
    }

    public void initWolrdData(int worldId) {
        if (!hasWorld(worldId)) {
        ...
        // check
        if (worldData.get(1) == null) {
            worldData.put(1, new WorldData());
        }
...
//        }
    ...
    private boolean hasWolrdMap(int mapId) {
    ...
    private void insertWolrdMap(WorldMap worldMap) {
        worldMapDao.insertWorldMap(worldMap);
    }

    private void initWorldMap() throws Exception {
        for (MapInfo mapInfo : worldMapInfo.values()) {
            if (mapInfo == null) {
                continue;
            }
            int mapId = mapInfo.getMapId();
            if (!hasWolrdMap(mapId)) {
                WorldMap worldMap = new WorldMap();
                worldMap.setMapId(mapId);
                insertWolrdMap(worldMap);
            }
        }
    ...
    private void loadWorldMap() throws Exception {
        List<WorldMap> worldMaps = worldMapDao.selectWorldMapList();
        if (worldMaps == null) {
            LogHelper.CONFIG_LOGGER.error("worldMaps is null!");
            return;
        }

        // 开始加载
        for (WorldMap worldMap : worldMaps) {
            int mapId = worldMap.getMapId();
            MapInfo mapInfo = worldMapInfo.get(mapId);
            if (mapInfo == null) {
                continue;
            }
            ...
            // 加载战斗数据
            battleManager.loadWar(worldMap, mapInfo);
//			readQuaickWar(worldMap, mapInfo);
//			readFarWar(worldMap, mapInfo);
//			readCountryWar(worldMap, mapInfo);
//			readBigMonsterWar(worldMap, mapInfo);
//			readZergWar(worldMap, mapInfo);
        }
    }
    ...
    private void readWorldData(WorldMap worldMap, MapInfo mapInfo) throws Exception {
        ...
        for (DataPb.MonsterData monsterData : serMapInfo.getMonsterList()) {
            ...
            if (mapInfo.getCityInfos().get(pos) != null || mapInfo.getEntity(pos) != null) {
            ...
            switch (monsterData.getEntityType()) {
                ...
                case EntityType.BIG_MONSTER:
                    ...
                    if (bigMonster.getState() == EntityState.DEATH.get()) {
                        ...
                        mapInfo.addPos(clonePos, bigMonster);
                        playerManager.clearPos(clonePos);
                        bigMonsterMap.put(clonePos, bigMonster);
                    }
                    break;
            }
        }
    ...
    public void initResource(SerMapInfo serMapInfo, MapInfo mapInfo) {
        ...
        for (DataPb.ResourceData resourceData : serMapInfo.getResourceList()) {
            ...
            if (mapInfo.getCityInfos().containsKey(resource.getPos())) {
            ...
            mapInfo.addPos(resource.getPos(), resource);
    ...
    public void initSuperResource(SerMapInfo serMapInfo, MapInfo mapInfo) {
        ...
        superMineList.forEach(superMine -> {
            ...
            if (staticSuperRes != null) {
                ...
                if (pos != null) {
                    resource.setPos(new Pos(pos.getX(), pos.getY()));
                }
    ...
    public void flushTaskMonster1(Player player, int taskId) {
        ...
        //List<Pos> flushPos = getTaskFlushMonsterPos(mapInfo, player);
        ////if (flushPos.size() != 2) {
        ////	logger.error("flushTaskMonster1 error player {} taskId {}");
        ////}
        //List<Entity> list = new ArrayList<>();
        ...
        Pos pos = getFlushTaskMosterPos(player);
        List<Entity> list = new ArrayList<>();
        if (pos != null) {
            Monster monster = addMonster(pos, generateMonster + 1000, generateMonster, mapInfo, AddMonsterReason.FLUSH_TASK_MONSTER);
            list.add(monster);
    ...
    public Pos getFlushTaskMosterPos(Player player) {
        ...
        for (int i = posx - 2; i <= posx + 2; i++) {
            for (int j = posy - 2; j <= posy + 2; j++) {
                ...
                if (mapInfo.isFreePos(elem)) {
                    elem.setDistance(player.getPos());

                    if (pos == null) {
                        pos = elem;
                    } else {
                        if (elem.getDistance() < pos.getDistance()) {
                            pos = elem;
                        }
                    }
                }
...
//        }
    ...
    public Monster addMonster(Pos pos, int monsterId, int monsterLv, MapInfo mapInfo, int reason) {
        Pos clonePos = pos.clone();
        Map<Pos, Monster> monsterMap = mapInfo.getMonsterMap();
        Monster monster = createMonster(monsterId, monsterLv, clonePos, reason);
        mapInfo.addPos(clonePos, monster);
        playerManager.clearPos(clonePos);
        monsterMap.put(clonePos, monster);
    ...
    public Monster addRiotMonster(Pos pos, int monsterId, int monsterLv, MapInfo mapInfo, int reason) {
        Pos clonePos = pos.clone();
        Map<Pos, Monster> monsterMap = mapInfo.getMonsterMap();
        Monster monster = createRiotMonster(monsterId, monsterLv, clonePos);
        mapInfo.addPos(clonePos, monster);
        playerManager.clearPos(clonePos);
        monsterMap.put(clonePos, monster);
        mapInfo.updateMonsterNum(monsterLv);
    ...
    public BigMonster addBigMonster(Pos pos, long monsterId, int monsterLv, MapInfo mapInfo, int reason) {
        ...
        BigMonster monster = createBigMonster(monsterId, monsterLv, clonePos);
        mapInfo.addMonsterPos(clonePos, monster);
        playerManager.clearPos(clonePos);
        monsterMap.put(clonePos, monster);
        return monster;
    }
    ...
    public RebelMonster addRebelMonster(Pos pos, int monsterId, int monsterLv, MapInfo mapInfo, int reason, int country) {
        ...
        RebelMonster monster = createRebelMonster(monsterId, monsterLv, clonePos, reason, country);
        mapInfo.addPos(clonePos, monster);
        playerManager.clearPos(clonePos);
        monsterMap.put(clonePos, monster);
        mapInfo.updateMonsterNum(monsterLv);
    ...
    public Resource addResource(Pos pos, int resLv, int resType, MapInfo mapInfo) {
        ...
        resource.setPos(clonePos);
        mapInfo.addPos(clonePos, resource);
        playerManager.clearPos(clonePos);
        resourceMap.put(clonePos, resource);
        return resource;
    }
    ...
    public void caculatePlayer(Team team, Player player, HashMap<Integer, Integer> solderRecMap) {
        ...
        if (soilder > 0) {
            ...
            seasonService.addTreasuryScore(player, GrandType.TYPE_2, 1, soilder);
        }
    }
    ...
    public void handleSoldierRec(HashMap<Long, HashMap<Integer, Integer>> solderRecMap, Hero hero, BattleEntity battleEntity) {
        ...
        if (diff > 0) {
            ...
            if (null != player) {
                ...
                double seasonBuf = seasonManager.getSeasonBuf(player, EffectType.EFFECT_TYPE9);// 赛季
                int rec = (int) ((soldierAdd + buff + seasonBuf) * (float) diff);
    ...
    public Pos givePlayerPos(MapInfo mapInfo) {
        ...
        if (worldMap == null) {
            LogHelper.CONFIG_LOGGER.error("worldMap is null, mapId = " + mapId);
            return new Pos();
        }

        int tryTimes = 0;
        while (!isPosOk(pos, worldMap, mapInfo)) {
            pos = mapInfo.randPickPos();
            ++tryTimes;
            if (tryTimes > 5) {
                break;
    ...
    public boolean isPosOk(Pos pos, StaticWorldMap worldMap, MapInfo mapInfo) {
        int x1 = worldMap.getX1() + 1;
        int x2 = worldMap.getX2() - 1;
        int y1 = worldMap.getY1() + 1;
        int y2 = worldMap.getY2() - 1;

        boolean isRangeOk = pos.getX() >= x1 && pos.getX() <= x2 && pos.getY() >= y1 && pos.getY() <= y2;
        boolean isFree = mapInfo.isFreePos(pos);

        return isRangeOk && isFree;
    }
...
//        }
    ...
    public Pos handleSpecialFly(Player player, int mapId) {
        ...
        if (!ownedCity.isEmpty()) {
            ...
            if (!ownedPos.isError() || !mapInfo.isFreePos(ownedPos)) {
                return ownedPos;
            }
        }
    ...
    public Pos randCityPos(List<Integer> cityIds, MapInfo mapInfo) {
        ...
        // 防御性代码
        if (freePos.isEmpty()) {
            LogHelper.CONFIG_LOGGER.error("randCityPos free pos is empty!");
            return resPos;
        }
        ...
        StaticWorldMap worldMap = staticWorldMgr.getStaticWorldMap(mapInfo.getMapId());
        int tryTimes = 0;
        while (!isPosOk(randomPos, worldMap, mapInfo)) {
            randomPos = keys.get(random.nextInt(keys.size()));
            ++tryTimes;
            if (tryTimes > 5) {
                break;
            }
    ...
    public Award caculateResCount(March march, Resource resource, long period, Player player, boolean isBreak) {
        ...
        double seasonBuf = seasonManager.getSeasonBuf(player, EffectType.EFFECT_TYPE27);// EFFECT_TYPE27(27, "采集获得资源增加（百分比）"),
        // LogHelper.GAME_DEBUG.error("season add = " + season);
        double totalAdd = techAddRes + addFactor + seasonBuf + 1.0;
    ...
    // 玩家切换地图[隐藏实现细节]
    public void changePlayerPos(Player player, MapInfo oldMapInfo, Pos newPos) {
        Pos playerPos = player.getPos();
        removePlayerCity(playerPos, oldMapInfo);
        int newMapId = getMapId(newPos);
        MapInfo newMapInfo = getMapInfo(newMapId);
        playerManager.changePlayerPos(player, newPos);
        addPlayerCity(newPos.clone(), newMapInfo, player);
    }
