...
//		}
        ...
        if (beforeLevel != lord.getLevel()) {
            ...
            activityEventManager.activityTip(EventEnum.UP_LEVEL, player, 0, 0);
            updateMasterAward(player);
            achievementService.addAndUpdate(player, AchiType.AT_1, lord.getLevel());
        }
    ...
    public void updateMasterAward(Player player) {
        Map<Long, Friend> master = friendService.getMaster(player);
        Friend friend = master.values().stream().findFirst().orElse(null);
        if (friend != null) {
            long rolaId = friend.getRolaId();
            Player player1 = playerManager.getPlayer(rolaId);
            if (player1 != null) {
                Map<Long, Map<Integer, Integer>> friAward = player1.getFriAward();
                Map<Integer, Integer> integerIntegerMap = friAward.computeIfAbsent(player.roleId, x -> new HashMap<>());
                ...
                appMap.values().forEach(x -> {
                    ...
                    if (integer == null && player.getLevel() >= x.getCond()) {
    ...
    public boolean subEnergy(Lord lord, int energy, int reason) {
        ...
        logUser.energyLog(EnergyLog.builder()
                ...
                .nick(lord.getNick())
                .reason(reason).build());
        Player player = playerManager.getPlayer(lord.getLordId());
        achievementService.addAndUpdate(player,AchiType.AT_62,energy);
        return true;
    }
...
//                lord.setsGameTimes(lord.getsGameTimes() + (staticVip.getGameTime() - beforeStaticVip.getGameTime()));
        ...
        if (vip != beforeVip) {
            ...
            achievementService.addAndUpdate(player,AchiType.AT_9,vip);
        }
        personalityManager.isPush(lord.getLordId(), vip);
        //获取美女
