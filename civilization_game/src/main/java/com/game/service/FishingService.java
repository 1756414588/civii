...
public class FishingService {
	 ...
	 * 抽奖方法
	 *
	 * @param orignalRates 中奖概率列表，保证顺序和实际物品对应
	 * @return 中奖商品索引
	 */
...
//                }
	...
	public void throwPoleRq(ThrowPoleRq req, ClientHandler handler) {
		...
		// 定义概率数组
		List<Double> orignalRates = new ArrayList<>(bait.getBaitToFishProbability().size());
		// 定义鱼类数组
		...
		for (Map.Entry<Integer, StaticBaitToFishProbability> entry : bait.getBaitToFishProbability().entrySet()) {
			...
			orignalRates.add(probability / 100.0);
			fishIds.add(entry.getValue().getFishId());
		}
		// 抽奖
		int res = lottery(orignalRates);

		// 获取鱼种
		StaticFish fish = staticFishMgr.getFishMap().get(fishIds.get(res));
	...
	public void takeBackPoleRq(TakeBackPoleRq req, ClientHandler handler) {
		...
		if (result == 0) {
			...
			if (critStatus == 1) {
				// 定义概率数组
				List<Double> orignalRates = new ArrayList<>(fishingLevel.getHit().size());
				// 定义暴击数组
				...
				for (Map.Entry<Integer, StaticFishLevelCritBuff> entry : fishingLevel.getHit().entrySet()) {
					int probability = entry.getValue().getProbability();
					orignalRates.add(probability / 100.0);
					multiple.add(entry.getValue().getMultiple());
				}
				// 抽奖
				int res = lottery(orignalRates);

				// 增加获得的鱼的数量  基础数值*暴击倍数
				record.setCount(record.getCount() * fishingLevel.getHit().get(multiple.get(res)).getMultiple());
				// 增加获得的积分  基础数值*暴击倍数
				record.setPoints(record.getPoints() * fishingLevel.getHit().get(multiple.get(res)).getMultiple());
			}
