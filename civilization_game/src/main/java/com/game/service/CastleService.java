...
public class CastleService {
	...
	public void getMiningHeroListRq(CastlePb.GetMiningHeroListRq re, ClientHandler handler) {
		...
		if (re.getArmyType() == CastleConsts.MINING) {
			player.setMiningList(playerManager.createMiningList(player.getLevel()));
		} else {
			player.setDefenseArmyList(playerManager.createDefanceArmyList(player.getLevel()));
		}
...
//        }
		...
		for (int i = 0; i < staticMeetingTask.getMonsterNum(); i++) {
			...
			while (count <= (num - CastleConsts.MEETING_MONSTER_CELL_NUM)) {
				monsterPos = worldManager.randPos(player, CastleConsts.MEETING_MONSTER_CELL_NUM + count);
				count++;
